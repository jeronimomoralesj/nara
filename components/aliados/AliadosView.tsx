              ...
              >
                <div className="grid lg:grid-cols-12">
                  ...
                  >
                    ...
                    {partner.founding && (
                      <span className="absolute left-6 top-6 inline-flex items-center gap-1.5 rounded-full bg-white/90 px-3 py-1 text-xs font-semibold text-terracotta-700 backdrop-blur">
                        <BadgeCheck size={14} />
                        Socio Fundador
                      </span>
                    )}
                  ...
                  <div className="p-8 sm:p-10 lg:col-span-7">
                    <div className="flex flex-wrap items-center gap-2">
                      {partner.badges.map((badge) => (
                        <span
                          key={badge}
                          className="inline-flex items-center gap-1.5 rounded-full bg-terracotta-50 px-3 py-1 text-xs font-medium text-terracotta-700"
                        >
                          <Sparkles size={12} />
                          {badge}
                        </span>
            ...
            {Array.from({ length: partnerSlots }).map((_, i) => (
              <Reveal key={i} delay={i * 0.08}>
                <motion.a
                  ...
                  whileHover={{ y: -6 }}
                  transition={{ duration: 0.4, ease: [0.22, 1, 0.36, 1] }}
                  className="group flex min-h-[240px] flex-col items-center justify-center rounded-3xl border border-dashed border-charcoal/20 bg-white/40 p-8 text-center transition-colors hover:border-terracotta-300 hover:bg-white"
                >
                  <span className="flex h-14 w-14 items-center justify-center rounded-full border border-charcoal/15 text-charcoal-muted transition-all duration-500 group-hover:rotate-90 group-hover:border-terracotta-400 group-hover:text-terracotta-500">
                    <Plus size={24} />
                  </span>
